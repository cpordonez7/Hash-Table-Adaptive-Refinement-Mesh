function [List_Neighbors_left_return] = getNeighbors_left_id_dif_level(HTObj,id)
    ...
    if (ind_i == 0) && (ind_j == 0)             
        ...
        %CELL NULL
        List_Neighbors_left_return(1,:) = 0; % only the id cells
        List_Neighbors_left_return(2,:) = 0; % only the level difference
    end
    ...
    if (0 < ind_i && ind_i < N_level-1) && (ind_j == 0)                 
        ...
        List_Neighbors_left_return(1,:) = List_Neighbors_left; % only the id cells
        List_level_difference = [];
        for j=1:length(List_Neighbors_left_return(1,:))
            id_j = List_Neighbors_left_return(1,j);

            cell_id_j = HTObj.getCell_id(id_j);         
            List_level_difference = [List_level_difference cell_id_j.l-l]; % only the level difference 
        end
        List_Neighbors_left_return(2,:) = List_level_difference;  % only the level difference
    end
