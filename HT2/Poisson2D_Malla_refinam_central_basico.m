...
%TEST: MALLAS REFINAMIENTOS BÁSICOS, EN NXM CON N=M
%Modificar archivo y ejem de EDP
clear
 grid.a1 = 0.0;
 grid.a2 = 0.0;
 grid.b1 = 1.0;
...
fileID1 = fopen('Result_numer_Poisson_2D_MALLA_REFINA_BASICOS.txt','w');
fprintf(fileID1,'\t\t RESULTADOS NUMERICOS POISSON 2D MALLAS REFINAMIENTO BÁSICO \n\n');
...
%loop for doubling of N and M
 for RepIni=1:Rep_Final
     ...
     if RepIni==1
         ...
         %Malla con refinamiento central básico
         ObjHT = New_Mesh2D('malla_4_refinada.txt',2,grid);
         % ObjHT = New_Mesh2D('malla_4x4_ref_basico.txt',2,grid); %Es el
         % mismo archivo
