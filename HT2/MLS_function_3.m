function[A,b, num_max_points]  = MLS_function_3(T,ObjHT,A,b,ID_CELL_CENT,Id_cel_vecina,pesoh,pesok,m,dl)
    %This function do the calculate the coefficientes the Matrix the system
    %for aplication of the DF
    %MLS_function_3 is the function with other case for the choose points
    %whit the neighboors the a point
    ...
    if dl>=1
        % disp('si')
        Vecinos_Id3 = ObjHT.getNeighbors_Punto(P1,dl);
        ...
        %%%ACTIVE this line when you want to apply MLS1
        if size(Vecinos_Id3,2) < 6
            Vecinos_Id3 = Add_cell_neighbors(ObjHT,Vecinos_Id3);
        end

        % This is the list with neighbors of neighbors not repeated
        %%ACTIVE this line and the line below when you want to apply MLS2 
        % % Vecinos_Id3 = Add_cell_neighbors(ObjHT,Vecinos_Id3);
        ...
        % % % % ACTIVE this line when you want to apply MLS2(Este metodo no se lo consideró)
    ...
    num_max_points = size(Point_list,2);
