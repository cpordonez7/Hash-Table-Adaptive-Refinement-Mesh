...
fileID1 = fopen('Result_numer_Poisson_2D.txt','w');
...
%loop for doubling of N and M
 for RepIni=1:Rep_Final
    ...
    profile off
     ...
     % Aprox = bicg(A,B,1e-10,450);   %gradiente biconjugado
     % Aprox = bicgstab(A,B,1e-10,450);

     %carga precondicionadores
    % setup = struct('type','ilutp','droptol',1e-6);
    % [L,U] = ilu(A,setup);
    % Aprox= bicg(A,B,1e-12,100,L,U);
